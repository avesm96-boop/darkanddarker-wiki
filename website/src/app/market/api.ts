...
export async function fetchMarketTrends(
  ...
  return ourGet<OurTrendsResponse>(`/market/trends?${params}`, signal);
}

// ---------------------------------------------------------------------------
// Market Deals — from our API
// ---------------------------------------------------------------------------
...
export async function fetchMarketDeals(
  category: string = "",
  signal?: AbortSignal,
): Promise<DealItem[]> {
  const params = category ? `?category=${category}` : "";
  const data = await ourGet<{ deals: DealItem[]; count: number }>(`/market/deals${params}`, signal);
  return data.deals;
}

// ---------------------------------------------------------------------------
// Market Fastest Selling — from our API
// ---------------------------------------------------------------------------

export interface FastestItem {
  item_marketplace_id: string;
  item_base_name: string;
  sold_count: number;
...
export async function fetchMarketFastest(
  category: string = "",
  hours: number = 24,
  signal?: AbortSignal,
): Promise<FastestItem[]> {
  ...
  const data = await ourGet<{ items: FastestItem[]; count: number }>(`/market/fastest?${params}`, signal);
  return data.items;
}

// ---------------------------------------------------------------------------
// Market Volume — from our API
// ---------------------------------------------------------------------------
...
export async function fetchMarketVolume(
  ...
  if (category) params.set("category", category);
  params.set("hours", String(hours));
  const data = await ourGet<{ items: VolumeItem[]; count: number }>(`/market/volume?${params}`, signal);
  return data.items;
}

// ---------------------------------------------------------------------------
// Market Supply Watch — from our API
// ---------------------------------------------------------------------------
...
export async function fetchMarketSupply(
  category: string = "",
  hours: number = 24,
  signal?: AbortSignal,
): Promise<{ draining: SupplyItem[]; flooding: SupplyItem[] }> {
  const params = new URLSearchParams();
  if (category) params.set("category", category);
  params.set("hours", String(hours));
  return ourGet<{ draining: SupplyItem[]; flooding: SupplyItem[] }>(`/market/supply?${params}`, signal);
}

// ---------------------------------------------------------------------------
// Market Price Spreads — from our API
// ---------------------------------------------------------------------------
...
export async function fetchMarketSpreads(
  category: string = "",
  signal?: AbortSignal,
): Promise<SpreadItem[]> {
  const params = category ? `?category=${category}` : "";
  const data = await ourGet<{ items: SpreadItem[]; count: number }>(`/market/spreads${params}`, signal);
  return data.items;
}

// ---------------------------------------------------------------------------
// RMT — from our API (endpoints already exist)
// ---------------------------------------------------------------------------
...
export interface RmtStats {
  ...
  top_rmt_items: Array<{
    ...
    total_rmt_gold: number;
  }>;
}

export async function fetchRmtStats(signal?: AbortSignal): Promise<RmtStats> {
  return ourGet<RmtStats>("/rmt/stats", signal);
}

export interface RmtSeller {
  seller_info: string;
...
export async function fetchRmtSellers(limit: number = 10, signal?: AbortSignal): Promise<RmtSeller[]> {
  const data = await ourGet<{ sellers: RmtSeller[]; count: number }>(`/rmt/sellers?limit=${limit}`, signal);
  return data.sellers;
}
